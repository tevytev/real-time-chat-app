...
export default function ChatRoom(props) {
  const { activeRoomId, loadingRooms } = props;
    ...
    if (scrollPosition === 0 && messages.length >= 20) {
      const fetchMoreMessages = async () => {
        try {
          ...
          const response = await axios.get(`${MESSAGE_URL}${activeRoomId}?skip=${skip}`, {
            headers: {
          ...
          <header className="chat-window-header">
            <div className="chat-room-info-container">
              <div className="chat-room-info-pfp">
                {activeRoomUsers[0].username[0].toUpperCase()}
              </div>
              <div className="chat-room-info-text-container">
                <h2>{activeRoomUsers[0].username}</h2>
              </div>
            </div>
          </header>
          <div id="chat-window" ref={chatWindowRef} className="chat-window-scroll-container">
            {loadingMoreMessages ? (<div className="loading-more-chat-window-scroll-container">
              ...
              messages.map((messageObj, index) => {
                if (messageObj.user !== user.userId) {
                  ...
                  return (
                    <>
                      <ReceivedMessage
                        username={activeRoomUsers[0].username}
                        key={index}
                        time={formattedTime}
